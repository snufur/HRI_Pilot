...
3. 正式实验：与5个不同的聊天机器人进行对话
   - 每个机器人进行20次对话
   - 每次对话后对礼貌程度评分（-3到3分）
   - 每组对话结束后完成主观感受问卷（10题）

重要提醒：
• 代入到所给的情境中，用你习惯的方式与机器人对话。
• 所有数据记录可查，不要在对话时直接复制情境文本！
• 聊天机器人的回复需要时间，请耐心等待。
• 认真思考每个问题，不要快速作答。
• 请根据真实感受评分，没有标准答案。
• 实验过程中可以随时休息，但禁止关闭当前页面。
...
预计总时长：90~120分钟`;
    ...
    async getPretestQuestionnaire() {
        const instruction = `接下来，我们会请您评估不同刺激具备某些能力的程度。
请根据 1 到 7 分的量表进行评估
1 = 完全沒有，7 = 非常高
请选择一个数字以表示您的回答。
